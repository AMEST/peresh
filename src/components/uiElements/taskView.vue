  ...
  <div>
      ...
      <div class="card-body">
        <div align="right">
          ...
          <button
            ...
            data-placement="bottom"
            v-bind:title="$Lang.editTask"
            v-on:click="setRework"
          >{{ $Lang.rework }}</button>

          <div class="dropdown pr-2 mr-2" v-if="customStatuses.length != 0">
            <button
              class="btn btn-outline-secondary dropdown-toggle btn-sm border-0 info-but-size"
              type="button"
              id="dropdownMenuButton"
              data-toggle="dropdown"
              aria-haspopup="true"
              aria-expanded="false"
            >{{ $Lang.changeToCustomStatus }}</button>
            <div class="dropdown-menu shadow" aria-labelledby="dropdownMenuButton">
              <a
                v-for="(item,key) in customStatuses"
                :key="item,key"
                class="dropdown-item"
                href="#"
                @click="setCustomStatus(item)"
              >{{ item }}</a>
            </div>
          </div>
          ...
          <button
            ...
            data-placement="bottom"
            v-bind:title="$Lang.editTask"
            @click="activeMenuItem = 'taskEditView'"
          >
            {{ $Lang.editTask }}
    ...
    <div class="row mt-3">
      ...
      <div v-if="currentTask.status" class="col">
        {{ $Lang.status.status }}:
        <span
          v-if="currentTask.customStatus && currentTask.status == 'do'"
          class="float-right badge badge-warning"
        >{{ currentTask.customStatus }}</span>
        <span
          v-else-if="currentTask.status == 'do'"
          class="float-right badge badge-secondary"
        >{{ $Lang.status[currentTask.status] }}</span>
    ...
    <div class="row mt-2">
      <div class="col">
        {{ $Lang.creationDate }}:
        <span class="float-right">{{ getCreated() }}</span>
      </div>
      <div class="col">
        {{ $Lang.endDate }}:
      ...
      <div class="col">
        <div class="progress">
          <div
            ...
            role="progressbar"
            v-bind:class="[taskEstimation.bg]"
            v-bind:style="[taskEstimation.style]"
          >{{ taskEstimation.days }} {{$Lang.days}}</div>
        </div>
      </div>
    </div>
    <!--todo-->
    ...
    <div class="row ml-3">
      <div v-if="currentTask.summary" class="col" style="min-height: 350px;">
...
export default {
  ...
  computed: {
    ...
    taskEstimation: {
      get: function() {
        var max =
          new Date(parseInt(this.currentTask.expiry)) -
          new Date(parseInt(this.currentTask.created));
        ...
        var background =
          currentPercent < 35
            ...
            : "bg-danger";
        background =
          this.currentTask.status === "archiv" ? "bg-success" : background;
        return {
          style: {
    ...
    hasCustomStatus: {
      get: function() {
        return (
          this.currentTask.customStatus !== undefined &&
          this.currentTask.customStatus !== "undefined"
        );
      }
    }
